...
def draw_landmarks(keypoints, h, w, enable_parts):
        
    image = np.zeros((h, w, 3))
    PART = PARTS
    for name, indices, color in PART:
        # 选择当前部分的关键点
        indices = np.array(indices) - 1
        current_part_keypoints = keypoints[indices]

        if (1 in enable_parts and "FACE" in name) or (2 in enable_parts and "EYEBROW" in name) or (3 in enable_parts and "NOSE" in name) or (4 in enable_parts and "LEFT_EYE" == name) or (4 in enable_parts and "RIGHT_EYE" == name) or (5 in enable_parts and "LIPS_OUTER" in name) or (6 in enable_parts and "LIPS_INNER" in name):
