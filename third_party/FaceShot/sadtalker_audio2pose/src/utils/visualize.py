...
def save_landmarks_to_video(landmarks, output_video='landmarks_video.mp4', frame_width=512, frame_height=512, fps=15):
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    video_writer = cv2.VideoWriter(output_video, fourcc, fps, (frame_width, frame_height))

    for i in range(landmarks.shape[0]):
        # 创建一个空白的RGB图像用于绘制
        img = draw_landmarks(landmarks[i], frame_width, frame_height)
        # 将绘制的图像写入视频
