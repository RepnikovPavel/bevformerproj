...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while raw_image is None or text is None or np.array(raw_image).shape[-1] < 3:
            # corner cases
...
class IPAdapter(torch.nn.Module):
    ...
    def __init__(self, unet, style_image_proj_modules, adapter_modules, ckpt_path=None):
        super().__init__()
        self.num_tokens = 6
        self.unet = unet
    ...
    def forward(self, noisy_latents, timesteps, encoder_hidden_states, style_image_embeds):
        style_ip_tokens = []
        # style tokens
...
def main():
    ...
    style_aware_encoder.cnn.requires_grad_(True)
    style_aware_encoder.position_embedding.requires_grad_(True)
    style_aware_encoder.style_embeddings.requires_grad_(True)
    ...
    ip_adapter = IPAdapter(unet, style_image_proj_models, adapter_modules, args.pretrained_ip_adapter_path)
    ...
    # style_image_encoder.to(accelerator.device, dtype=weight_dtype)

    # optimizer
    params_to_opt = itertools.chain(style_aware_encoder.position_embedding.parameters(),
                                    style_aware_encoder.cnn.parameters(),
                                    style_aware_encoder.style_embeddings.parameters(),
                                    ip_adapter.style_image_proj_modules.parameters(),
    ...
    ip_adapter, style_aware_encoder, optimizer, train_dataloader = accelerator.prepare(ip_adapter, style_aware_encoder, optimizer, train_dataloader)
    global_step = 0
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with (accelerator.accumulate(ip_adapter)):
                ...
                noise_pred = ip_adapter(noisy_latents, timesteps, encoder_hidden_states, style_embeds)
