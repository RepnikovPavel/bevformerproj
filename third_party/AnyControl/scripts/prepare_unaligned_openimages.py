...
def check_synsets(word):
    ...
    while True:
        if len(synsets_list) == 0:
            break
        curr = synsets_list.pop(0)
        checked[curr] = True
        if isinstance(curr, str):
            synsets = wn.synsets(word, pos='n')
            for syn in synsets:
                if syn not in checked:
                    synsets_list.append(syn)
        else:
            synsets = curr.hypernyms()
            for syn in synsets:
                if syn not in checked:
                    synsets_list.append(syn)
    synsets = checked.keys()
    for syn in synsets:
        if isinstance(syn, str) and syn in discard:
            return False
        if isinstance(syn, nltk.corpus.reader.wordnet.Synset) and syn.name() in discard:
            return False
    return True
...
if __name__ == "__main__":
    ...
    # generate
    image_source = os.path.join(args.data_root, "OpenImages/train/")
    mask_source = os.path.join(args.data_root, "OpenImages/train-masks/")
    anno_json = os.path.join(args.data_root, "OpenImages/train-annotations-object-segmentation.csv")
    label_json = os.path.join(args.data_root, "OpenImages/oidv7-class-descriptions-boxable.csv")

    save_json = os.path.join(args.data_root, "OpenImages/inpaint.json")
    output = os.path.join(args.data_root, "OpenImages/inpaint/")
    mask_output = os.path.join(args.data_root, "OpenImages/mask/")
    ...
    labels = {}
    lines = open(label_json).read().splitlines()[1:]
    for line in lines:
        if line == '': continue
        cls_id, name = line.split(',')
        keep = check_synsets(name)
        labels[cls_id] = dict(name=name, keep=keep)

    lines = open(anno_json).read().splitlines()[1:]
    for line in tqdm(lines):
        if line == '': continue

        mask_id, image_id, labelname = line.split(',')[:3]
        mask_path = os.path.join(mask_source, mask_id)
        image_path = os.path.join(image_source, image_id+'.jpg')
        cls_label = labels[labelname]
        if not cls_label['keep']:
            continue

        mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
        h, w = mask.shape[:2]
        ratio = (mask>0).sum() / (h*w)
        ...
        maskf = deepcopy(mask) / 255.
        kernel = np.ones((5, 5), np.uint8)
        mask = cv2.dilate(mask, kernel, iterations = 10)
        mask = Image.fromarray(mask)

        input_image = {'image': image, 'mask': mask}
        inpaint_result, gallery = infer(input_image)

        output_path = output + mask_id
        mask_output_path = mask_output + mask_id
        ret = cv2.resize(np.array(inpaint_result[1]), (w, h))
        saved_mask = cv2.cvtColor(cv2.resize((maskf*255).astype(np.uint8), (w, h)), cv2.COLOR_GRAY2RGB)
        Image.fromarray(ret).save(output_path)
        Image.fromarray(saved_mask).save(mask_output_path)

        save_info = {
            'image_id': image_id+'.jpg',
            'class': cls_label['name'],
            'mask': mask_id,
            'inpaint': mask_id,
        }
